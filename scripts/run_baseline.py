"""
Full baseline pipeline: MOT17/MOT20 -> Grounding DINO -> SAM 3.1
-> IoU tracking (+ Qwen3-VL running alongside).

Only run this after test_dataset.py, check_gpu.py, run_detection.py,
run_qwen_test.py, and a standalone SAM 3.1 test have each succeeded
individually — see README.md, section "Recommended run order".
...
def main():
    ...
    if config["models"]["sam31"]["enabled"]:
        ...
        sam = SAM31Model(
            checkpoint_path=sam_config["checkpoint"],
            device=device,
        )
